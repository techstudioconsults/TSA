...
import { Input, SelectSeparator } from "../../atoms";
...
export const TsaFooter: FC<TsaFooterProperties> = ({ className, navLinks, subscribeComponent, ...rest }) => {
  return (
    <div className={cn(`bg-primary px-[1rem] py-[64px] text-white lg:px-0`, className)} {...rest}>
        ...
        <section className="flex flex-col gap-8 lg:flex-row lg:justify-between lg:gap-0">
              ...
              {navLinks?.map((item) => {
                if (item.dropdown) {
                  return item.dropdown.map((course, index) => {
                    return (
                      <li key={index}>
                        <Link className="text-xs text-white hover:text-mid-danger" href={course.href}>
                          {course.title}
                        </Link>
                      </li>
                    );
          ...
          <div className="flex flex-col lg:w-1/6">
            ...
            <ul className="mt-3 flex flex-col gap-3">
              <li>
                <Link className="text-xs text-white hover:text-mid-danger" href="/about">
                  Our Story
                </Link>
              </li>
              <li>
                <Link className="text-xs text-white hover:text-mid-danger" href="/contact">
                  Contact Us
                </Link>
              </li>
              <li>
                <Link className="text-xs text-white hover:text-mid-danger" href="/careers">
                  Careers
                </Link>
              </li>
            </ul>
          ...
          <div className="flex flex-col lg:w-1/3">
            <div>
              <h5 className="pb-2.5 text-base text-white">Subscribe to our newsletter</h5>
              {subscribeComponent || (
