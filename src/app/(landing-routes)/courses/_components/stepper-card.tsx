    ...
    <section className={`${course.bgColor} py-[70px]`}>
      <Wrapper className="flex flex-col-reverse items-end justify-between lg:flex-row">
          ...
          <div className="relative">
            <div
              className={cn(
                ...
                course.id % 2 == 0 ? `border-low-warning bg-mid-warning` : `border-low-blue bg-mid-blue`,
              )}
            ></div>
            <h6 className={`text-[12px] font-[700] uppercase ${ifBgColorPrimary}`}>{course.caption}</h6>
            <h3 className={`text-[20px] font-[700] lg:text-[33px] ${ifBgColorPrimary}`}>{course.title}</h3>
          </div>
          <p className={`mb-[50px] mt-[16px] text-sm leading-[28px] ${ifBgColorPrimary}`}>{course.desc}</p>
        ...
        <div className="hidden flex-1 lg:block">
          <Image width={450} height={400} src={course.img} alt="gifs" className="ml-auto" />
        </div>
      </Wrapper>
    </section>
  );
